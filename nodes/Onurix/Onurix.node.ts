		...
		properties: [
			...
			// Contact Fields
			{
				...
				displayOptions: {
					show: {
						resource: ['contact'],
						operation: ['create', 'update', 'delete', 'associateGroup', 'disassociateGroup'],
					},
				},
				default: '',
			...
			try {
				...
				if (resource === 'call') {
					if (operation === 'send') {
						...
						const voice = this.getNodeParameter('voice', i, 'Mariana') as string;
						const audioCode = this.getNodeParameter('audioCode', i, '') as string;
						const groups = this.getNodeParameter('groups', i, '') as string;
				...
				if (resource === 'whatsapp') {
					if (operation === 'sendTemplate') {
						const fromPhoneMetaId = this.getNodeParameter('fromPhoneMetaId', i) as string;
						const phone = this.getNodeParameter('phone', i) as string;
						...
						const messageValue = this.getNodeParameter('messageValue', i) as string;
